...
import { i as buildAgentMainSessionKey, u as normalizeAgentId } from "./session-key-IUFoWh21.js";
...
const loadOverviewModule = async () => await import("./overview-PNfpg445.js");
const loadModelsSharedModule = async () => await import("./shared-CXkd0n7M.js");
const loadConfigCliModule = async () => await import("./config-cli-BNUdqyZu.js");
const loadDoctorModule = async () => await import("./doctor-nMNbHoQ4.js");
...
async function runGatewayLifecycle(operation) {
	const lifecycle = await import("./lifecycle-CJDTaayr.js");
	if (operation === "start") {
...
async function executeCrestodianOperation(operation, runtime, opts = {}) {
	...
	if (operation.kind === "plugin-search") {
		...
		await (opts.deps?.runPluginsSearch ?? (async (query, pluginRuntime) => {
			const { runPluginsSearchCommand } = await import("./plugins-search-command-Dhd-BFmb.js");
	...
	if (operation.kind === "plugin-uninstall") {
		...
		await (opts.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
			const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-BDtT_2LF.js");
	...
	if (operation.kind === "create-agent") {
		...
		const before = await readConfigFileSnapshot();
		const workspace = resolveUserPath(operation.workspace ?? process.cwd());
		await (opts.deps?.runAgentsAdd ?? (await import("./agents.commands.add-CREenyAB.js")).agentsAddCommand)({
	...
	if (operation.kind === "status") {
		...
		const { statusCommand } = await import("./status.command-t2_Fi0cm.js");
	...
	if (operation.kind === "open-tui") {
		...
		const result = await (opts.deps?.runTui ?? (await import("./tui-CgVzlbBT.js")).runTui)({
			local: true,
			session,
			deliver: false,
			historyLimit: 200
